...
def my_complaints(user: dict = Depends(require_user)) -> list[dict]:
    result = (
        get_db().table("complaints")
        .select("*,categories(name,key),subcategories(name)")
...
def analyze_complaint(complaint_id: str, user: dict = Depends(require_user)) -> dict[str, Any]:
    ...
    if incident_id:
        ...
        incident = db.table("incidents").insert({
            ...
            "building": complaint.get("building"),
            "block": complaint.get("block"),
            "floor": complaint.get("floor"),
            ...
            "first_reported_at": complaint["submitted_at"],
